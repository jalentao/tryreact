...
var LinkedStateMixin = {
  /**
   * Create a ReactLink that's linked to part of this components's state. The
...
function validateMethodOverride(proto, name) {
  ...
  if (proto.hasOwnProperty(name)) {
    !(specPolicy === SpecPolicy.DEFINE_MANY || specPolicy === SpecPolicy.DEFINE_MANY_MERGED) ? "development" !== 'production' ? invariant(false, 'ReactClassInterface: You are attempting to define ' + '`%s` on your components more than once. This conflict may be due ' + 'to a mixin.', name) : invariant(false) : undefined;
  }
}
...
function mixSpecIntoComponent(Constructor, spec) {
  ...
  !(typeof spec !== 'function') ? "development" !== 'production' ? invariant(false, 'ReactClass: You\'re attempting to ' + 'use a components class as a mixin. Instead, just use a regular object.') : invariant(false) : undefined;
  !!ReactElement.isValidElement(spec) ? "development" !== 'production' ? invariant(false, 'ReactClass: You\'re attempting to ' + 'use a components as a mixin. Instead, just use a regular object.') : invariant(false) : undefined;
    ...
    if (RESERVED_SPEC_KEYS.hasOwnProperty(name)) {
      ...
      if (shouldAutoBind) {
        ...
        if (isAlreadyDefined) {
          ...
          !(isReactClassMethod && (specPolicy === SpecPolicy.DEFINE_MANY_MERGED || specPolicy === SpecPolicy.DEFINE_MANY)) ? "development" !== 'production' ? invariant(false, 'ReactClass: Unexpected spec policy %s for key %s ' + 'when mixing in components specs.', specPolicy, name) : invariant(false) : undefined;
...
function mixStaticSpecIntoComponent(Constructor, statics) {
  ...
  for (var name in statics) {
    ...
    !!isInherited ? "development" !== 'production' ? invariant(false, 'ReactClass: You are attempting to define ' + '`%s` on your components more than once. This conflict may be ' + 'due to a mixin.', name) : invariant(false) : undefined;
  ...
  if ("development" !== 'production') {
    ...
    boundMethod.bind = function (newThis) {
      ...
      if (newThis !== component && newThis !== null) {
        "development" !== 'production' ? warning(false, 'bind(): React components methods may only be bound to the ' + 'components instance. See %s', componentName) : undefined;
      } else if (!args.length) {
        "development" !== 'production' ? warning(false, 'bind(): You are binding a components method to the components. ' + 'React does this for you automatically in a high-performance ' + 'way, so you can safely remove this call. See %s', componentName) : undefined;
        return boundMethod;
      }
...
var ReactClass = {
  ...
  createClass: function (spec) {
    var Constructor = function (props, context, updater) {
      ...
      if ("development" !== 'production') {
        "development" !== 'production' ? warning(this instanceof Constructor, 'Something is calling a React components directly. Use a factory or ' + 'JSX instead. See: https://fb.me/react-legacyfactory') : undefined;
      }
    ...
    if ("development" !== 'production') {
      "development" !== 'production' ? warning(!Constructor.prototype.componentShouldUpdate, '%s has a method called ' + 'componentShouldUpdate(). Did you mean shouldComponentUpdate()? ' + 'The name is phrased as a question because the function is ' + 'expected to return a value.', spec.displayName || 'A components') : undefined;
      "development" !== 'production' ? warning(!Constructor.prototype.componentWillRecieveProps, '%s has a method called ' + 'componentWillRecieveProps(). Did you mean componentWillReceiveProps()?', spec.displayName || 'A components') : undefined;
    }
...
/**
 * If your React components's render function is "pure", e.g. it will render the
  ...
  mountComponent: function (rootID, transaction, context) {
    ...
    if ("development" !== 'production') {
      ...
      if (inst.render == null) {
        ...
        "development" !== 'production' ? warning(Component.prototype && Component.prototype.isReactComponent || !canInstantiate || !(inst instanceof Component), '%s(...): React components classes must extend React.Component.', Component.displayName || Component.name || 'Component') : undefined;
      }
    }
    ...
    if ("development" !== 'production') {
      ...
      "development" !== 'production' ? warning(!inst.propTypes, 'propTypes was defined as an instance property on %s. Use a static ' + 'property to define propTypes instead.', this.getName() || 'a components') : undefined;
      "development" !== 'production' ? warning(!inst.contextTypes, 'contextTypes was defined as an instance property on %s. Use a ' + 'static property to define contextTypes instead.', this.getName() || 'a components') : undefined;
      "development" !== 'production' ? warning(typeof inst.componentShouldUpdate !== 'function', '%s has a method called ' + 'componentShouldUpdate(). Did you mean shouldComponentUpdate()? ' + 'The name is phrased as a question because the function is ' + 'expected to return a value.', this.getName() || 'A components') : undefined;
      "development" !== 'production' ? warning(typeof inst.componentDidUnmount !== 'function', '%s has a method called ' + 'componentDidUnmount(). But there is no such lifecycle method. ' + 'Did you mean componentWillUnmount()?', this.getName() || 'A components') : undefined;
      "development" !== 'production' ? warning(typeof inst.componentWillRecieveProps !== 'function', '%s has a method called ' + 'componentWillRecieveProps(). Did you mean componentWillReceiveProps()?', this.getName() || 'A components') : undefined;
    }
...
function assertValidProps(component, props) {
  ...
  if ("development" !== 'production') {
    ...
    "development" !== 'production' ? warning(!props.contentEditable || props.children == null, 'A components is `contentEditable` and contains `children` managed by ' + 'React. It is now your responsibility to guarantee that none of ' + 'those nodes are unexpectedly modified or duplicated. This is ' + 'probably not intentional.') : undefined;
  }
...
/**
 * ReactLink encapsulates a common pattern in which a components wants to modify
...
var ReactMount = {
  ...
  findComponentRoot: function (ancestorNode, targetID) {
    ...
    if ("development" !== 'production') {
      ...
      "development" !== 'production' ? warning(deepestAncestor != null, 'React can\'t find the root components node for data-reactid value ' + '`%s`. If you\'re seeing this message, it probably means that ' + 'you\'ve loaded two copies of React on the page. At this time, only ' + 'a single copy of React can be loaded at a time.', targetID) : undefined;
    }
...
function createInternalComponent(element) {
  !genericComponentClass ? "development" !== 'production' ? invariant(false, 'There is no registered components for the tag %s', element.type) : invariant(false) : undefined;
...
/**
 * ReactPropTransferer are capable of transferring props to another components
...
var ReactStateSetters = {
   ...
   * of that to set the components's state.
   *
...
var ReactTestUtils = {
  ...
  findAllInRenderedTree: function (inst, test) {
    ...
    !ReactTestUtils.isCompositeComponent(inst) ? "development" !== 'production' ? invariant(false, 'findAllInRenderedTree(...): instance must be a composite components') : invariant(false) : undefined;
  ...
  /**
   * Pass a mocked components module to this method to augment it with
   * useful methods that allow it to be used as a dummy React components.
   * Instead of rendering as usual, the components will become a simple
   * <div> containing any provided children.
   *
   * @param {object} module the mock function object exported from a
   *                        module that defines the components to be mocked
...
ReactShallowRenderer.prototype.render = function (element, context) {
  !ReactElement.isValidElement(element) ? "development" !== 'production' ? invariant(false, 'ReactShallowRenderer render(): Invalid components element.%s', typeof element === 'function' ? ' Instead of passing a components class, make sure to instantiate ' + 'it by passing it to React.createElement.' : '') : invariant(false) : undefined;
...
var ReactTransitionGroup = React.createClass({
  ...
  performLeave: function (key) {
    ...
    if (component.componentWillLeave) {
      ...
      // again, effectively mutating the components before all the work
      // is done.
...
function getInternalInstanceReadyForUpdate(publicInstance, callerName) {
  ...
  if (!internalInstance) {
    if ("development" !== 'production') {
      ...
      "development" !== 'production' ? warning(!callerName, '%s(...): Can only update a mounted or mounting components. ' + 'This usually means you called %s() on an unmounted components. ' + 'This is a no-op. Please check the code for the %s components.', callerName, callerName, publicInstance.constructor.displayName) : undefined;
    }
    return null;
  }
...
function findDOMNode(componentOrElement) {
  ...
  !(componentOrElement.render == null || typeof componentOrElement.render !== 'function') ? "development" !== 'production' ? invariant(false, 'findDOMNode was called on an unmounted components.') : invariant(false) : undefined;
